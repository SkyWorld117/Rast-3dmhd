...
struct SimState {
  explicit SimState(const Derived& d) : d(d) {
    ...
    ru = Field(g, 0.0); rv = Field(g, 0.0); rw = Field(g, 0.0);
    ro = Field(g, 0.0); tt = Field(g, 0.0);
    uu = Field(g, 0.0); vv = Field(g, 0.0); ww = Field(g, 0.0);
    fu = Field(g, 0.0); fv = Field(g, 0.0); fw = Field(g, 0.0);
    fr = Field(g, 0.0); ft = Field(g, 0.0);
    zru = Field(g, 0.0); zrv = Field(g, 0.0); zrw = Field(g, 0.0);
    zro = Field(g, 0.0); ztt = Field(g, 0.0);
    ww1 = Field(g, 0.0); ww2 = Field(g, 0.0); ww3 = Field(g, 0.0);
    bx = Field(g, 0.0); by = Field(g, 0.0); bz = Field(g, 0.0);
    zbx = Field(g, 0.0); zby = Field(g, 0.0); zbz = Field(g, 0.0);
    exx.resize(d.nx); dxxdx.resize(d.nx); d2xxdx2.resize(d.nx); ddx.resize(d.nx);
    wyy.resize(d.ny); dyydy.resize(d.ny); d2yydy2.resize(d.ny); ddy.resize(d.ny);
    zee.resize(d.nz); dzzdz.resize(d.nz); d2zzdz2.resize(d.nz); ddz.resize(d.nz);
    rkapa.resize(d.nz); dkapa.resize(d.nz);
  }
