...
namespace {
    ...
    double max_err_cols(const Field& f, const std::vector<double>& g, const Params& p, int ilap) {
    ...
    for (int k = p.ilap / 2; k <= nz - 1 - p.ilap / 2; ++k)
        for (int j = p.iy / 2; j <= ny - p.iy / 2 - 1; ++j)
            for (int i = p.ix / 2; i <= nx - p.ix / 2 - 1; ++i)
                ww1err = std::max(
                    ww1err,
                    std::fabs(s.ww1.at(i, j, k) -
                              g.f3d[Gstate::idx3d("ww1")][(long) k * nx * ny + (long) j * nx + i]));
