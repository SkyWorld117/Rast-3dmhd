...
TEST(Grid, GoldenYJacobiRank0And1) {
    ...
    for (int mypey = 0; mypey < 2; ++mypey) {
        std::string path = golden_dir() + "/gstate.preloop.00" + std::to_string(mypey) + ".0000";
        Gstate g         = read_gstate(path);
        ASSERT_EQ(g.mypey, mypey);
        std::vector<double> wyy(kNY), dyydy(kNY), d2yydy2(kNY), ddy(kNY);
        yjacobi(p, mypey, /*npey=*/2, kNY, kNPY, kNRY, wyy.data(), dyydy.data(), d2yydy2.data(),
                ddy.data());
        ...
        for (int j = 0; j < kNY; ++j) {
            ...
            EXPECT_DOUBLE_EQ(d2yydy2[j], g_d2yydy2[j])
                << "mypey=" << mypey << " d2yydy2[" << j << "]";
...
TEST(Grid, GoldenZZacobiRank0And2) {
    ...
    for (int mypez : {0, 1}) {
        ...
        std::string path = golden_dir() + "/gstate.preloop.00" + std::to_string(rank) + ".0000";
        Gstate g         = read_gstate(path);
        ASSERT_EQ(g.mypez, mypez);
        std::vector<double> zee(kNZ), dzzdz(kNZ), d2zzdz2(kNZ), ddz(kNZ);
        zjacobi(p, mypez, /*npez=*/2, kNZ, kNPZ, kNRZ, zee.data(), dzzdz.data(), d2zzdz2.data(),
                ddz.data());
        ...
        for (int k = 0; k < kNZ; ++k) {
            ...
            EXPECT_DOUBLE_EQ(d2zzdz2[k], g_d2zzdz2[k])
                << "mypez=" << mypez << " d2zzdz2[" << k << "]";
