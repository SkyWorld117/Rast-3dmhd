...
TEST(Communicate, PeriodicFieldSurvivesExchange) {
    ...
    for (int k = 0; k < d.nz; ++k)
        for (int j = 0; j < d.ny; ++j)
            for (int i = 0; i < d.nx; ++i)
                s.ru.at(i, j, k) = truth_fn(gx_of(p, i), gy_of(p, t, j), gz_of(p, t, k));
    ...
    for (int k = 0; k < d.nz; ++k)
        for (int j = 0; j < d.ny; ++j)
            for (int i = 0; i < d.nx; ++i) {
                double want = truth_fn(gx_of(p, i), gy_of(p, t, j), gz_of(p, t, k));
...
TEST(Communicate, AllPrimaryFieldsCommunicate) {
    ...
    auto periodic_fill = [&](Field& f, double mult) {
        for (int k = 0; k < d.nz; ++k)
            for (int j = 0; j < d.ny; ++j)
                for (int i = 0; i < d.nx; ++i)
                    f.at(i, j, k) = mult * truth_fn(gx_of(p, i), gy_of(p, t, j), gz_of(p, t, k));
    };
    ...
    auto check = [&](const Field& f, double mult, const char* name) {
        ...
        for (int k = 0; k < d.nz; ++k)
            for (int j = 0; j < d.ny; ++j)
                for (int i = 0; i < d.nx; ++i) {
                    double want = mult * truth_fn(gx_of(p, i), gy_of(p, t, j), gz_of(p, t, k));
...
TEST(Communicate, HorizontalMeanOfSinusoidalVerticalProfile) {
    ...
    for (int k = 0; k < d.nz; ++k)
        for (int j = 0; j < d.ny; ++j)
            for (int i = 0; i < d.nx; ++i) s.uu.at(i, j, k) = 1.0 + 0.5 * std::cos(0.1 * k);
