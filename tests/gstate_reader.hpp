...
inline int Gstate::idx3d(const std::string& name) {
  static const char* names[27] = {
      "ru",  "rv",  "rw",  "ro",  "tt",  "uu",  "vv",  "ww",
      "fu",  "fv",  "fw",  "fr",  "ft",  "zru", "zrv", "zrw",
      "zro", "ztt", "ww1", "ww2", "ww3", "bx",  "by",  "bz",
      "zbx", "zby", "zbz"};
  for (int i = 0; i < 27; ++i)
...
inline int Gstate::idx1d(const std::string& name) {
  static const char* names[14] = {
      "exx",   "dxxdx", "d2xxdx2", "ddx", "wyy", "dyydy", "d2yydy2",
      "ddy",   "zee",   "dzzdz",   "d2zzdz2", "ddz", "rkapa", "dkapa"};
  for (int i = 0; i < 14; ++i)
    if (name == names[i]) return i;
  return -1;
}
