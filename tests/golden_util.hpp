...
namespace r3dtest {
    ...
    inline void load_state(r3d::SimState& s, const Gstate& g) {
        ...
        r3d::Field* dst[27] = {&s.ru,  &s.rv,  &s.rw,  &s.ro,  &s.tt,  &s.uu,  &s.vv,
        ...
        Q q[14] = {{"exx", &s.exx},         {"dxxdx", &s.dxxdx},     {"d2xxdx2", &s.d2xxdx2},
        ...
        for (auto& e : q)
            e.dst->assign(g.f1d[Gstate::idx1d(e.name)].begin(), g.f1d[Gstate::idx1d(e.name)].end());
        s.dt    = g.scalars[0];
        s.timc  = g.scalars[1];
        s.timt  = g.scalars[2];
        s.timi  = g.scalars[3];
    ...
    inline double max_err_interior(const r3d::Field& f, const std::vector<double>& g, int ix,
                                   int iy, int ilap) {
        ...
        for (int k = ilap / 2; k <= nz - 1 - ilap / 2; ++k)
            for (int j = 1; j <= ny - iy; ++j)
                for (int i = 1; i <= nx - ix; ++i)
                    m = std::max(
                        m, std::fabs(f.at(i, j, k) - g[(long) k * nx * ny + (long) j * nx + i]));
        return m;
    }
