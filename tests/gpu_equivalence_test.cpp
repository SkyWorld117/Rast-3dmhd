...
TEST(Gpu, EquivalenceWithCpu) {
  ...
  gpu_upload(g1sim, g1);
  SimState g1sc(d);
  g1sc.tb = g1.tb; g1sc.timi = g1.timi;
  ...
  for (const char* nm : {"ru", "rv", "rw", "ro", "tt"}) {
    ...
    if (e != 0.0) {
      ...
      fprintf(stderr, "GPU-vs-GOLDEN %s rank %d bad=%ld (boundary-planes=%ld) argmax e=%.3e at (i=%d j=%d k=%d) gpu=%.17g gold=%.17g\n",
    ...
    if (nm[0]=='r' && nm[1]=='u' && rank==0) {
      ...
      fprintf(stderr, "RU rank0 row: k=1 gpu=%.9g gold=%.9g | k=8 gpu=%.9g gold=%.9g | k=15 gpu=%.9g gold=%.9g | k=16 gpu=%.9g gold=%.9g | fu(8,4,8) gpu=%.6g gold=%.6g | ww1 gpu=%.6g gold=%.6g\n",
  ...
  for (const char* nm : {"ru", "rv", "rw", "ro", "tt"}) {
    ...
    if (nm[0] == 'r' && nm[1] == 'u') { f_cpu = &cpu.ru; f_gpu = &gpu_state.ru; }
    if (nm[0] == 'r' && nm[1] == 'v') { f_cpu = &cpu.rv; f_gpu = &gpu_state.rv; }
    if (nm[0] == 'r' && nm[1] == 'w') { f_cpu = &cpu.rw; f_gpu = &gpu_state.rw; }
    if (nm[0] == 'r' && nm[1] == 'o') { f_cpu = &cpu.ro; f_gpu = &gpu_state.ro; }
    if (nm[0] == 't') { f_cpu = &cpu.tt; f_gpu = &gpu_state.tt; }
    double err = max_err_interior(*f_cpu, std::vector<double>(f_gpu->data(),
                                   f_gpu->data() + f_gpu->size()),
                                  p.ix, p.iy, p.ilap);
