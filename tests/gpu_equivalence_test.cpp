...
TEST(Gpu, StageByStageVsGolden) {
    ...
    for (int st = 1; st <= 3; ++st) {
        ...
        for (const char* nm : {"ru", "rv", "rw", "ro", "tt"}) {
            const Field* f = nullptr;
            if (nm[0] == 'r' && nm[1] == 'u') f = &s.ru;
            if (nm[0] == 'r' && nm[1] == 'v') f = &s.rv;
            if (nm[0] == 'r' && nm[1] == 'w') f = &s.rw;
            if (nm[0] == 'r' && nm[1] == 'o') f = &s.ro;
            if (nm[0] == 't') f = &s.tt;
            {
                ...
                if (st == 1) {
                    ...
                    fprintf(stderr,
                            "WW1 rank %d: gpu(k=15)=%.9g gold=%.9g | gpu(k=16)=%.9g gold=%.9g | "
                            "gpu(k=17)=%.9g gold=%.9g | ro(k=17) gpu=%.9g goldpre=%.9g\n",
                            rank, s.ww1.at(1, 1, 15),
                            f1p.f3d[Gstate::idx3d("ww1")][15 * 18 * 10 + 1 * 18 + 1],
                            s.ww1.at(1, 1, 16),
                            f1p.f3d[Gstate::idx3d("ww1")][16 * 18 * 10 + 1 * 18 + 1],
                            s.ww1.at(1, 1, 17),
                            f1p.f3d[Gstate::idx3d("ww1")][17 * 18 * 10 + 1 * 18 + 1],
                            s.ro.at(1, 1, 17), 0.0);
                }
                ...
                for (const char* nm : {"fu", "fv", "fw", "fr", "ft"}) {
                    ...
                    if (e != 0.0) {
                        ...
                        for (int k = p.ilap / 2; k <= nz - 1 - p.ilap / 2; ++k)
                            for (int j = 1; j <= ny - p.iy; ++j)
                                for (int i = 1; i <= nx - p.ix; ++i) {
                                    long idx = (long) k * nx * ny + (long) j * nx + i;
                                    double d2 =
                                        std::fabs(f->at(i, j, k) - fs.f3d[Gstate::idx3d(nm)][idx]);
                                    if (d2 > best) {
                                        best = d2;
                                        bi   = i;
                                        bj   = j;
                        ...
                        fprintf(
                            stderr,
                            "FLUX%d %s rank %d (i=%d j=%d k=%d) e=%.3e gpu=%.17g gold=%.17g\n", st,
                            nm, rank, bi, bj, bk, best, f->at(bi, bj, bk),
        ...
        if (rank <= 1) {
            Gstate ckj =
                read_gstate(golden_dir() + "/gstate.ck." + tag + ".000" + std::to_string(st));
            auto idx3 = [&](const char* n, int i, int j, int k) {
                return (long) k * d.nx * d.ny + (long) j * d.nx + i;
            };
            fprintf(stderr,
                    "YCOL st%d rank %d ru[i=8,k=8] j0 gpu=%.9g gold=%.9g | j8 gpu=%.9g gold=%.9g | "
                    "j9 gpu=%.9g gold=%.9g\n",
                    st, rank, s.ru.at(8, 0, 8), ckj.f3d[Gstate::idx3d("ru")][idx3("ru", 8, 0, 8)],
        ...
        if (st == 3) {
            ...
            fprintf(stderr,
                    "PST rank %d ru[8,4,16] gpu=%.9g gold=%.9g  ru[8,4,17] gpu=%.9g gold=%.9g  "
                    "ru[0,4,8] gpu=%.9g gold=%.9g  ru[17,4,8] gpu=%.9g gold=%.9g  ru[8,0,8] "
                    "gpu=%.9g gold=%.9g  ru[8,9,8] gpu=%.9g gold=%.9g\n",
                    rank, s.ru.at(8, 4, 16), ck.f3d[Gstate::idx3d(nm)][gidx(nm, 8, 4, 16)],
