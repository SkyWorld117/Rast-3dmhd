...
__global__ void k_ft_advect(const K, const double*, const double*, const double*,
...
__global__ void k_ft_diffuse(const K, const double*, const double*, double*, const double*,
                  ...
                  const double*, const double*, const double*);
__global__ void k_viscous_fu(const K, const double*, const double*, const double*,
                              double*, double*, double*, const double*,
                              const double*, const double*, const double*,
                              const double*, const double*);
__global__ void k_viscous_fv(const K, const double*, const double*, const double*,
                              double*, double*, double*, const double*,
                              const double*, const double*, const double*,
                              const double*, const double*);
__global__ void k_viscous_fw(const K, const double*, const double*, const double*,
                              double*, double*, double*, const double*,
                              const double*, const double*, const double*,
                              const double*, const double*);
...
__global__ void k_rot(const K, double*, double*, double*, const double*, const double*,
           const double*);
__global__ void k_step_uu(const K, const double*, const double*, const double*,
               const double*, const double*, double*);
__global__ void k_step_dt(const K, const double*, const double*, const double*,
...
void gpu_alloc(GpuSim& g, const Derived& d) {
  ...
  alloc(&g.ru, n); alloc(&g.rv, n); alloc(&g.rw, n);
  alloc(&g.ro, n); alloc(&g.tt, n);
  alloc(&g.uu, n); alloc(&g.vv, n); alloc(&g.ww, n);
  alloc(&g.fu, n); alloc(&g.fv, n); alloc(&g.fw, n);
  alloc(&g.fr, n); alloc(&g.ft, n);
  alloc(&g.zru, n); alloc(&g.zrv, n); alloc(&g.zrw, n);
  alloc(&g.zro, n); alloc(&g.ztt, n);
  alloc(&g.ww1, n); alloc(&g.ww2, n); alloc(&g.ww3, n);
  alloc(&g.bx, n); alloc(&g.by, n); alloc(&g.bz, n);
  alloc(&g.zbx, n); alloc(&g.zby, n); alloc(&g.zbz, n);
  alloc(&g.exx, d.nx); alloc(&g.dxxdx, d.nx); alloc(&g.d2xxdx2, d.nx);
  alloc(&g.ddx, d.nx);
  alloc(&g.wyy, d.ny); alloc(&g.dyydy, d.ny); alloc(&g.d2yydy2, d.ny);
  alloc(&g.ddy, d.ny);
  alloc(&g.zee, d.nz); alloc(&g.dzzdz, d.nz); alloc(&g.d2zzdz2, d.nz);
  alloc(&g.ddz, d.nz);
  alloc(&g.rkapa, d.nz); alloc(&g.dkapa, d.nz);
}
...
#define FREE(x)                     \
  ...
  FREE(g.ru); FREE(g.rv); FREE(g.rw); FREE(g.ro); FREE(g.tt);
  FREE(g.uu); FREE(g.vv); FREE(g.ww); FREE(g.fu); FREE(g.fv); FREE(g.fw);
  FREE(g.fr); FREE(g.ft); FREE(g.zru); FREE(g.zrv); FREE(g.zrw); FREE(g.zro);
  FREE(g.ztt); FREE(g.ww1); FREE(g.ww2); FREE(g.ww3); FREE(g.bx); FREE(g.by);
  FREE(g.bz); FREE(g.zbx); FREE(g.zby); FREE(g.zbz);
  FREE(g.exx); FREE(g.dxxdx); FREE(g.d2xxdx2); FREE(g.ddx);
  FREE(g.wyy); FREE(g.dyydy); FREE(g.d2yydy2); FREE(g.ddy);
  FREE(g.zee); FREE(g.dzzdz); FREE(g.d2zzdz2); FREE(g.ddz);
  FREE(g.rkapa); FREE(g.dkapa);
#undef FREE
}
...
void gpu_upload(GpuSim& g, const SimState& s) {
  ...
  cp(g.ru, s.ru.data(), n); cp(g.rv, s.rv.data(), n); cp(g.rw, s.rw.data(), n);
  cp(g.ro, s.ro.data(), n); cp(g.tt, s.tt.data(), n);
  cp(g.uu, s.uu.data(), n); cp(g.vv, s.vv.data(), n); cp(g.ww, s.ww.data(), n);
  cp(g.fu, s.fu.data(), n); cp(g.fv, s.fv.data(), n); cp(g.fw, s.fw.data(), n);
  cp(g.fr, s.fr.data(), n); cp(g.ft, s.ft.data(), n);
  cp(g.zru, s.zru.data(), n); cp(g.zrv, s.zrv.data(), n);
  cp(g.zrw, s.zrw.data(), n); cp(g.zro, s.zro.data(), n); cp(g.ztt, s.ztt.data(), n);
  cp(g.ww1, s.ww1.data(), n); cp(g.ww2, s.ww2.data(), n); cp(g.ww3, s.ww3.data(), n);
  cp(g.bx, s.bx.data(), n); cp(g.by, s.by.data(), n); cp(g.bz, s.bz.data(), n);
  cp(g.zbx, s.zbx.data(), n); cp(g.zby, s.zby.data(), n); cp(g.zbz, s.zbz.data(), n);
  cp(g.exx, s.exx.data(), g.nx); cp(g.dxxdx, s.dxxdx.data(), g.nx);
  cp(g.d2xxdx2, s.d2xxdx2.data(), g.nx); cp(g.ddx, s.ddx.data(), g.nx);
  cp(g.wyy, s.wyy.data(), g.ny); cp(g.dyydy, s.dyydy.data(), g.ny);
  cp(g.d2yydy2, s.d2yydy2.data(), g.ny); cp(g.ddy, s.ddy.data(), g.ny);
  cp(g.zee, s.zee.data(), g.nz); cp(g.dzzdz, s.dzzdz.data(), g.nz);
  cp(g.d2zzdz2, s.d2zzdz2.data(), g.nz); cp(g.ddz, s.ddz.data(), g.nz);
  cp(g.rkapa, s.rkapa.data(), g.nz); cp(g.dkapa, s.dkapa.data(), g.nz);
}

void gpu_download(SimState& s, GpuSim& g) {
  long n = (long)g.nx * g.ny * g.nz;
  cp_back(s.ru.data(), g.ru, n); cp_back(s.rv.data(), g.rv, n);
  cp_back(s.rw.data(), g.rw, n); cp_back(s.ro.data(), g.ro, n);
  cp_back(s.tt.data(), g.tt, n); cp_back(s.uu.data(), g.uu, n);
  cp_back(s.vv.data(), g.vv, n); cp_back(s.ww.data(), g.ww, n);
  cp_back(s.fu.data(), g.fu, n); cp_back(s.fv.data(), g.fv, n);
  cp_back(s.fw.data(), g.fw, n); cp_back(s.fr.data(), g.fr, n);
  cp_back(s.ft.data(), g.ft, n); cp_back(s.zru.data(), g.zru, n);
  cp_back(s.zrv.data(), g.zrv, n); cp_back(s.zrw.data(), g.zrw, n);
  cp_back(s.zro.data(), g.zro, n); cp_back(s.ztt.data(), g.ztt, n);
  cp_back(s.ww1.data(), g.ww1, n); cp_back(s.ww2.data(), g.ww2, n);
  cp_back(s.ww3.data(), g.ww3, n); cp_back(s.bx.data(), g.bx, n);
  cp_back(s.by.data(), g.by, n); cp_back(s.bz.data(), g.bz, n);
  cp_back(s.zbx.data(), g.zbx, n); cp_back(s.zby.data(), g.zby, n);
  cp_back(s.zbz.data(), g.zbz, n);
  cp_back(s.exx.data(), g.exx, g.nx); cp_back(s.dxxdx.data(), g.dxxdx, g.nx);
  cp_back(s.d2xxdx2.data(), g.d2xxdx2, g.nx); cp_back(s.ddx.data(), g.ddx, g.nx);
  cp_back(s.wyy.data(), g.wyy, g.ny); cp_back(s.dyydy.data(), g.dyydy, g.ny);
  cp_back(s.d2yydy2.data(), g.d2yydy2, g.ny); cp_back(s.ddy.data(), g.ddy, g.ny);
  cp_back(s.zee.data(), g.zee, g.nz); cp_back(s.dzzdz.data(), g.dzzdz, g.nz);
  cp_back(s.d2zzdz2.data(), g.d2zzdz2, g.nz); cp_back(s.ddz.data(), g.ddz, g.nz);
  cp_back(s.rkapa.data(), g.rkapa, g.nz); cp_back(s.dkapa.data(), g.dkapa, g.nz);
}
...
void comm_field_gpu(MPI_Comm comm, const Params& p, const Topology& t,
                    GpuSim& g, double* f) {
  ...
  if (t.npez > 1) {
    ...
    if (t.mypez == 0) {
      ...
      MPI_Recv(f + (long)(nz - ilaph) * nx * ny, vcount, MPI_DOUBLE,
               t.mype + t.npey, tag2, comm, MPI_STATUS_IGNORE);
    } else if (t.mypez == t.npez - 1) {
      MPI_Recv(f, vcount, MPI_DOUBLE, t.mype - t.npey, tag1, comm,
               MPI_STATUS_IGNORE);
      MPI_Send(f + (long)ilaph * nx * ny, vcount, MPI_DOUBLE, t.mype - t.npey,
               tag2, comm);
    } else {
  ...
  static long ycap = 0;
  if (ycap < ycount) {
    if (ysend) { cudaFree(ysend); cudaFree(yrecv); }
  ...
  if (t.npey > 1) {
    ...
    if (t.mypey == 0) {
      k_pack_y<<<ceil_div(ycount, kThreads), kThreads>>>(ysend, f, nx, ny,
                                                         ny - p.iy, iyh, nz);
      check();
      MPI_Send(ysend, ycount, MPI_DOUBLE, t.mype + 1, tag3, comm);
      MPI_Recv(yrecv, ycount, MPI_DOUBLE, t.mype + 1, tag4, comm, MPI_STATUS_IGNORE);
      k_unpack_y<<<ceil_div(ycount, kThreads), kThreads>>>(f, yrecv, nx, ny,
                                                           ny - iyh, iyh, nz);
      check();
    } else if (t.mypey == t.npey - 1) {
      MPI_Recv(yrecv, ycount, MPI_DOUBLE, t.mype - 1, tag3, comm, MPI_STATUS_IGNORE);
      k_unpack_y<<<ceil_div(ycount, kThreads), kThreads>>>(f, yrecv, nx, ny, 0,
                                                           iyh, nz);
      check();
      k_pack_y<<<ceil_div(ycount, kThreads), kThreads>>>(ysend, f, nx, ny, iyh,
                                                         iyh, nz);
      check();
    ...
    k_pack_y<<<ceil_div(ycount, kThreads), kThreads>>>(ysend, f, nx, ny,
                                                       ny - p.iy, iyh, nz);
    check();
    k_unpack_y<<<ceil_div(ycount, kThreads), kThreads>>>(f, ysend, nx, ny, 0,
                                                         iyh, nz);
    check();
    k_pack_y<<<ceil_div(ycount, kThreads), kThreads>>>(ysend, f, nx, ny, iyh,
                                                       iyh, nz);
    check();
    k_unpack_y<<<ceil_div(ycount, kThreads), kThreads>>>(f, ysend, nx, ny,
                                                         ny - iyh, iyh, nz);
    check();
  }
  ...
  k_pack_x<<<ceil_div(xcount, kThreads), kThreads>>>(xbuf, f, nx, ny, nx - p.ix,
                                                     p.ix / 2, nz);
  check();
  k_unpack_x<<<ceil_div(xcount, kThreads), kThreads>>>(f, xbuf, nx, ny, 0,
                                                       p.ix / 2, nz);
  check();
  k_pack_x<<<ceil_div(xcount, kThreads), kThreads>>>(xbuf, f, nx, ny, p.ix / 2,
                                                     p.ix / 2, nz);
  check();
  k_unpack_x<<<ceil_div(xcount, kThreads), kThreads>>>(f, xbuf, nx, ny,
                                                       nx - p.ix / 2, p.ix / 2, nz);
  check();
}
}
...
void step_gpu(MPI_Comm comm, const Params& p, const Derived& d, const Topology& t,
  ...
  K q;
  q.nx = g.nx; q.ny = g.ny; q.nz = g.nz;
  q.i1 = 1; q.i2 = g.nx - p.ix;
  q.j1 = 1; q.j2 = g.ny - p.iy;
  q.k1 = p.ilap / 2; q.k2 = g.nz - 1 - p.ilap / 2;
  q.c13 = d.c13; q.c23 = d.c23; q.c43 = d.c43; q.ore = d.ore; q.ocv = d.ocv;
  q.repr = d.repr; q.grav = p.grav;
  q.hx = d.hx; q.h2x = d.h2x; q.hy = d.hy; q.h2y = d.h2y;
  q.hz = d.hz; q.h2z = d.h2z;
  q.omx = d.omx; q.omz = d.omz;
  q.gamma = p.gamma; q.re = p.re; q.cv = d.cv;
  q.xp = p.xp; q.yp = p.yp; q.hh = p.hh;
  q.li = p.ilap / 2; q.lj = p.iy / 2; q.lk = p.ix / 2;
  ...
  CUDA_CHECK(cudaMemcpy(host_tmp, g.uu, (size_t)n * sizeof(double),
  ...
  k_step_dt<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                           (q.k2 - q.k1 + 1),
                       kThreads), kThreads>>>(q, g.ro, g.uu, g.rkapa, g.ddx, g.ddy,
                                              g.ddz, g.ww1, g.ww2, g.ww3);
  check();
  ...
  CUDA_CHECK(cudaMemcpy(host_tmp, g.ww1, (size_t)n * sizeof(double),
                        cudaMemcpyDeviceToHost));
  double wmin[4] = {1e300, 1e300, 1e300, 1e300};
  wmin[0] = interior_min(host_tmp, 1e300);
  CUDA_CHECK(cudaMemcpy(host_tmp, g.ww2, (size_t)n * sizeof(double),
                        cudaMemcpyDeviceToHost));
  wmin[1] = interior_min(host_tmp, 1e300);
  CUDA_CHECK(cudaMemcpy(host_tmp, g.ww3, (size_t)n * sizeof(double),
                        cudaMemcpyDeviceToHost));
  ...
  k_save_state<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.ru, g.rv, g.rw, g.ro, g.tt,
                                                 g.zru, g.zrv, g.zrw, g.zro, g.ztt);
  check();

  auto flux_gpu = [&]() {
    k_fr<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1),
                    kThreads), kThreads>>>(q, g.ru, g.rv, g.rw, g.fr, g.ww1, g.dxxdx,
                                           g.dyydy, g.dzzdz);
    if (t.mypez == 0)
    ...
    check();
    k_fr_subw1<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.fr, g.ww1);
    k_force<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1),
                       kThreads), kThreads>>>(q, p.grav, g.ro, g.tt, g.ro, g.fu,
                                              g.fv, g.fw, g.ww1, g.dxxdx, g.dyydy,
                                              g.dzzdz);
    k_vel<<<ceil_div(n, kThreads), kThreads>>>(q, g.ru, g.rv, g.rw, g.ro, g.uu,
                                               g.vv, g.ww);
    k_adv<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1),
                     kThreads), kThreads>>>(q, 0, g.ru, g.rv, g.rw, g.uu, g.vv,
                                            g.ww, g.fu, g.fv, g.fw, g.dxxdx, g.dyydy,
                                            g.dzzdz);
    check();
    k_adv<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1),
                     kThreads), kThreads>>>(q, 1, g.ru, g.rv, g.rw, g.uu, g.vv,
                                            g.ww, g.fu, g.fv, g.fw, g.dxxdx, g.dyydy,
                                            g.dzzdz);
    k_adv<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1),
                     kThreads), kThreads>>>(q, 2, g.ru, g.rv, g.rw, g.uu, g.vv,
                                            g.ww, g.fu, g.fv, g.fw, g.dxxdx, g.dyydy,
                                            g.dzzdz);
    check();
    k_ft_advect<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                               (q.k2 - q.k1 + 1),
                           kThreads), kThreads>>>(q, g.uu, g.vv, g.ww, g.tt, g.ft,
                                                  g.dxxdx, g.dyydy, g.dzzdz);
    k_ft_diffuse<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                (q.k2 - q.k1 + 1),
                            kThreads), kThreads>>>(q, g.ro, g.tt, g.ft, g.dxxdx,
                                                   g.d2xxdx2, g.dyydy, g.d2yydy2,
                                                   g.dzzdz, g.d2zzdz2, g.rkapa,
                                                   g.dkapa);
    check();
    k_viscous_fu<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                (q.k2 - q.k1 + 1),
                            kThreads), kThreads>>>(q, g.uu, g.vv, g.ww, g.fu, g.fv,
                                                   g.fw, g.dxxdx, g.d2xxdx2, g.dyydy,
                                                   g.d2yydy2, g.dzzdz, g.d2zzdz2);
    k_viscous_fv<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                (q.k2 - q.k1 + 1),
                            kThreads), kThreads>>>(q, g.uu, g.vv, g.ww, g.fu, g.fv,
                                                   g.fw, g.dxxdx, g.d2xxdx2, g.dyydy,
                                                   g.d2yydy2, g.dzzdz, g.d2zzdz2);
    k_viscous_fw<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                (q.k2 - q.k1 + 1),
                            kThreads), kThreads>>>(q, g.uu, g.vv, g.ww, g.fu, g.fv,
                                                   g.fw, g.dxxdx, g.d2xxdx2, g.dyydy,
                                                   g.d2yydy2, g.dzzdz, g.d2zzdz2);
    check();
    k_w1_w2_dx<<<ceil_div((long)(q.i2 - q.i1 + 1) * q.ny * (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.uu, g.vv, g.ww1, g.ww2,
                                                 g.dxxdx);
    k_w3_dx<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                           (q.k2 - q.k1 + 1),
                       kThreads), kThreads>>>(q, g.ww, g.ww3, g.dxxdx);
    k_ft_heat1<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.ro, g.tt, g.ft, g.ww1, g.ww2,
                                                 g.ww3);
    check();
    k_cross_fufv<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                (q.k2 - q.k1 + 1),
                            kThreads), kThreads>>>(q, g.fu, g.fv, g.ww1, g.ww2,
                                                   g.dyydy);
    k_w1_w2_dy<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.uu, g.vv, g.ww1, g.ww2,
                                                 g.dyydy);
    k_ft_heat2<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.ro, g.tt, g.ft, g.ww1, g.ww2);
    check();
    k_w1_dx_again<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                 (q.k2 - q.k1 + 1),
                             kThreads), kThreads>>>(q, g.uu, g.ww1, g.dxxdx);
    k_w3_dz_full<<<ceil_div((long)q.nx * q.ny * (q.k2 - q.k1 + 1), kThreads),
        kThreads>>>(q, g.ww, g.ww3, g.dzzdz);
    k_ft_heat3a<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                               (q.k2 - q.k1 + 1),
                           kThreads), kThreads>>>(q, g.ro, g.tt, g.ft, g.ww1, g.ww2,
                                                  g.ww3);
    check();
    k_cross_w3<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.fu, g.fv, g.ww3, g.dxxdx,
                                                 g.dyydy);
    k_w1_w2_dz<<<ceil_div((long)q.nx * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1) +
                              (long)(q.i2 - q.i1 + 1) * q.ny * (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.uu, g.vv, g.ww1, g.ww2,
                                                 g.dzzdz);
    k_w3_dy<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                           (q.k2 - q.k1 + 1),
                       kThreads), kThreads>>>(q, g.ww, g.ww3, g.dyydy);
    k_ft_heat3c<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                               (q.k2 - q.k1 + 1),
                           kThreads), kThreads>>>(q, g.ro, g.ft, g.ww1, g.ww2,
                                                  g.ww3);
    check();
    k_fw_cross<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                              (q.k2 - q.k1 + 1),
                          kThreads), kThreads>>>(q, g.fw, g.ww1, g.ww2, g.dxxdx,
                                                 g.dyydy);
    k_w3_dx_2<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                             (q.k2 - q.k1 + 1),
                         kThreads), kThreads>>>(q, g.ww, g.ww3, g.dxxdx);
    k_ft_heat3d<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                               (q.k2 - q.k1 + 1),
                           kThreads), kThreads>>>(q, g.ro, g.ft, g.ww1, g.ww3);
    if (p.lrot)
      k_rot<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                           (q.k2 - q.k1 + 1),
                       kThreads), kThreads>>>(q, g.fu, g.fv, g.fw, g.ru, g.rv, g.rw);
    check();
  };
  ...
  auto bcon_gpu = [&]() {
    ...
    if (t.mypez == 0)
      k_bcon<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1), kThreads),
          kThreads>>>(q, g.exx, g.wyy, g.dzzdz, g.rkapa, g.ru, g.rv, g.rw, g.ro, g.tt,
                      d.c13, d.c23, d.c43, p.theta, sc.tb, tpr, cln, dz, plume,
                      p.itcon, p.izcon, 1);
    check();
    if (t.mypez == t.npez - 1)
      k_bcon<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1), kThreads),
  ...
  auto substep_gpu = [&](double coef) {
    k_update_prim<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                                 (q.k2 - q.k1 + 1),
                             kThreads), kThreads>>>(q, coef, g.zru, g.zrv, g.zrw,
                                                    g.zro, g.ztt, g.fu, g.fv, g.fw,
                                                    g.fr, g.ft, g.ru, g.rv, g.rw,
                                                    g.ro, g.tt);
    check();
  };
  auto mix_gpu = [&](double coef) {
    k_mix_state<<<ceil_div((long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) *
                               (q.k2 - q.k1 + 1),
                           kThreads), kThreads>>>(q, coef, g.ru, g.rv, g.rw, g.ro,
                                                  g.tt, g.fu, g.fv, g.fw, g.fr, g.ft,
                                                  g.zru, g.zrv, g.zrw, g.zro, g.ztt);
    check();
  };
  auto comm_gpu = [&]() {
