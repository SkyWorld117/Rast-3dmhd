...
void fluxes(const Params& p, const Derived& d, const Topology& t,
            const double rkapa[], const double dkapa[], SimState& s) {
  ...
  if (p.ixcon != 0 || p.iycon != 0)
    unsupported("non-periodic horizontal boundaries");

  Field& RU = s.ru; Field& RV = s.rv; Field& RW = s.rw;
  Field& RO = s.ro; Field& TT = s.tt;
  Field& UU = s.uu; Field& VV = s.vv; Field& WW = s.ww;
  Field& FU = s.fu; Field& FV = s.fv; Field& FW = s.fw;
  Field& FR = s.fr; Field& FT = s.ft;
  Field& WW1 = s.ww1; Field& WW2 = s.ww2; Field& WW3 = s.ww3;
  Field& BX = s.bx; Field& BY = s.by; Field& BZ = s.bz;
  ...
  for (int k = k1; k <= k2; ++k)
    for (int j = j1; j <= j2; ++j)
      for (int i = i1; i <= i2; ++i)
        FW.at(i, j, k) = p.grav * RO.at(i, j, k);
  ...
  for (int k = 0; k < nz; ++k)
    for (int j = 0; j < ny; ++j)
      for (int i = 0; i < nx; ++i)
        UU.at(i, j, k) = RU.at(i, j, k) * RO.at(i, j, k);
  for (int k = 0; k < nz; ++k)
    for (int j = 0; j < ny; ++j)
      for (int i = 0; i < nx; ++i)
        VV.at(i, j, k) = RV.at(i, j, k) * RO.at(i, j, k);
  for (int k = 0; k < nz; ++k)
    for (int j = 0; j < ny; ++j)
      for (int i = 0; i < nx; ++i)
        WW.at(i, j, k) = RW.at(i, j, k) * RO.at(i, j, k);
  ...
  for (int k = k1; k <= k2; ++k) {
    ...
    for (int j = j1; j <= j2; ++j) {
      ...
      for (int i = i1; i <= i2; ++i) {
        ...
        FT.at(i, j, k) -= VV.at(i, j, k) *
                          (TT.at(i, j + 1, k) - TT.at(i, j - 1, k)) * tmpy;
        FT.at(i, j, k) -= WW.at(i, j, k) *
                          (TT.at(i, j, k + 1) - TT.at(i, j, k - 1)) * tmpz;
      }
    }
  }
      ...
      for (int i = i1; i <= i2; ++i) {
        ...
        FV.at(i, j, k) += ore * (
            ((VV.at(i + 1, j, k) - VV.at(i - 1, j, k)) * hx * s.d2xxdx2[i] +
             (VV.at(i + 1, j, k) - 2.0 * VV.at(i, j, k) + VV.at(i - 1, j, k)) *
                 h2x * s.dxxdx[i] * s.dxxdx[i]) +
        ...
        FW.at(i, j, k) += ore * (
            ((WW.at(i + 1, j, k) - WW.at(i - 1, j, k)) * hx * s.d2xxdx2[i] +
             (WW.at(i + 1, j, k) - 2.0 * WW.at(i, j, k) + WW.at(i - 1, j, k)) *
                 h2x * s.dxxdx[i] * s.dxxdx[i]) +
  ...
  // Cross terms coupling the horizontal shear to FU/FV.
  {
    ...
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FU.at(i, j, k) += tmp * (WW2.at(i, j + 1, k) - WW2.at(i, j - 1, k)) *
                            hy * s.dyydy[j];
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FV.at(i, j, k) += tmp * (WW1.at(i, j + 1, k) - WW1.at(i, j - 1, k)) *
                            hy * s.dyydy[j];
  }
...

  {
    ...
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FT.at(i, j, k) += (WW3.at(i, j, k) * WW3.at(i, j, k) -
                             WW1.at(i, j, k) * WW3.at(i, j, k) -
...

  {
    ...
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FU.at(i, j, k) += tmp * (WW3.at(i + 1, j, k) - WW3.at(i - 1, j, k)) *
                            hx * s.dxxdx[i];
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FV.at(i, j, k) += tmp * (WW3.at(i, j + 1, k) - WW3.at(i, j - 1, k)) *
                            hy * s.dyydy[j];
  }
...

  {
    ...
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FW.at(i, j, k) += tmp * ((WW1.at(i + 1, j, k) - WW1.at(i - 1, j, k)) *
                                       hx * s.dxxdx[i] +
                                   (WW2.at(i, j + 1, k) - WW2.at(i, j - 1, k)) *
                                       hy * s.dyydy[j]);
  }
        ...
        WW3.at(i, j, k) = (WW.at(i + 1, j, k) - WW.at(i - 1, j, k)) * hx * s.dxxdx[i];
  {
    ...
    for (int k = k1; k <= k2; ++k)
      for (int j = j1; j <= j2; ++j)
        for (int i = i1; i <= i2; ++i)
          FT.at(i, j, k) += 2.0 * WW1.at(i, j, k) * WW3.at(i, j, k) * RO.at(i, j, k) * tmp;
  }
