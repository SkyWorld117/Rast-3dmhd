...
namespace r3d {
    ...
    void rzextr(int iest, double xest, const double* yest, double* yz, double d[kNMax][kNuse],
                double* dy, int nv, double* xbuf) {
        xbuf[iest - 1] = xest;
        if (iest == 1) {
            for (int j = 0; j < nv; ++j) {
                yz[j]   = yest[j];
    ...
    void mmid(const Params& p, const double* y, const double* dydx, int nvar, double xs,
              double htot, int nstp, double* yout, DerivFn derivs) {
        double h = htot / nstp;
        ...
        for (int i = 0; i < nvar; ++i) yout[i] = 0.5 * (ym[i] + yn[i] + h * ytemp[i]);
    }

    void bsstep(const Params& p, double* y, double* dydx, int nv, double& x, double htry,
                double eps, double& hdid, double& hnext, DerivFn derivs, double* xbuf) {
        double h    = htry;
        double xsav = x;
        ...
        for (;;) {  // Fortran line 1: on failure, shrink and retry
            for (int i = 1; i <= kIMax; ++i) {
                ...
                if (getenv("ODE_DEBUG"))
                    fprintf(stderr, "i=%d y=(%.6g %.6g) yerr=(%.3g %.3g)\n", i, y[0], y[1], yerr[0],
                            yerr[1]);
                double errmax = 0.0;
                for (int j = 0; j < nv; ++j) {
    ...
    void static_profile(const Params& p, int npz, double* tprof, double* rprof) {
        ...
        for (int k = 1; k < npz; ++k) {
            ...
            double htry = dzz;
            double hdid, hnext;
            bsstep(p, y, dydx, nv, zz, htry, /*eps=*/1.0e-12, hdid, hnext, derivs_static, xbuf);
