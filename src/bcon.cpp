    ...
    void bcon(const Params& p, const Derived& d, const Topology& t, SimState& s) {
        ...
        if (t.mypez == 0) {
            ...
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i)
                    s.ru.at(i, j, k1) =
                        s.ro.at(i, j, k1) * (c43 * s.ru.at(i, j, k1 + 1) / s.ro.at(i, j, k1 + 1) -
                                             c13 * s.ru.at(i, j, k1 + 2) / s.ro.at(i, j, k1 + 2));
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i)
                    s.rv.at(i, j, k1) =
                        s.ro.at(i, j, k1) * (c43 * s.rv.at(i, j, k1 + 1) / s.ro.at(i, j, k1 + 1) -
                                             c13 * s.rv.at(i, j, k1 + 2) / s.ro.at(i, j, k1 + 2));
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i) s.rw.at(i, j, k1) = 0.0;
            ...
            if (p.lmag && p.ibcon == 0) {
                for (int j = j1; j <= j2; ++j)
                    for (int i = i1; i <= i2; ++i) s.bz.at(i, j, k1) = 0.0;
            }
        }
        ...
        if (t.mypez == t.npez - 1) {
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i)
                    s.ru.at(i, j, k2) =
                        s.ro.at(i, j, k2) * (c43 * s.ru.at(i, j, k2 - 1) / s.ro.at(i, j, k2 - 1) -
                                             c13 * s.ru.at(i, j, k2 - 2) / s.ro.at(i, j, k2 - 2));
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i)
                    s.rv.at(i, j, k2) =
                        s.ro.at(i, j, k2) * (c43 * s.rv.at(i, j, k2 - 1) / s.ro.at(i, j, k2 - 1) -
                                             c13 * s.rv.at(i, j, k2 - 2) / s.ro.at(i, j, k2 - 2));
            for (int j = j1; j <= j2; ++j)
                for (int i = i1; i <= i2; ++i) s.rw.at(i, j, k2) = 0.0;

            if (p.izcon == 0) {
                // Constant temperature lower boundary.
                for (int j = j1; j <= j2; ++j)
                    for (int i = i1; i <= i2; ++i) s.tt.at(i, j, k2) = s.tb;
            } else if (p.izcon == 1) {
            ...
            if (p.lmag && p.ibcon == 0) {
                for (int j = j1; j <= j2; ++j)
                    for (int i = i1; i <= i2; ++i) s.bz.at(i, j, k2) = 0.0;
            }
        }
    }
