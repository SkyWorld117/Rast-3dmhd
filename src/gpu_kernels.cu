...
__global__ void k_fr(const K q, const double* ru, const double* rv,
                     const double* rw, double* fr, double* ww1,
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    fr[f] = (ru[f3(q.nx, q.ny, i - 1, j, k)] - ru[f3(q.nx, q.ny, i + 1, j, k)]) *
            q.hx * dxxdx[i];
    fr[f] -= (rv[f3(q.nx, q.ny, i, j + 1, k)] - rv[f3(q.nx, q.ny, i, j - 1, k)]) *
             q.hy * dyydy[j];
    ww1[f] = (rw[f3(q.nx, q.ny, i, j, k + 1)] - rw[f3(q.nx, q.ny, i, j, k - 1)]) *
             q.hz * dzzdz[k];
  }
}
__global__ void k_fr_w1b(const K q, const double* rw, double* ww1,
                         const double* dzzdz, int top) {
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    long f = f3(q.nx, q.ny, i, j, k);
    ww1[f] = top
                 ? (4.0 * rw[f3(q.nx, q.ny, i, j, k + 1)] -
                    rw[f3(q.nx, q.ny, i, j, k + 2)]) * tz
                 : (rw[f3(q.nx, q.ny, i, j, k - 2)] -
                    4.0 * rw[f3(q.nx, q.ny, i, j, k - 1)]) * tz;
  }
}
...
__global__ void k_force(const K q, double grav, const double* ro0, const double* tt,
                        double* ro, double* fu, double* fv, double* fw,
                        double* ww1, const double* dxxdx, const double* dyydy,
                        const double* dzzdz) {
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    fu[f] = (ww1[f3(q.nx, q.ny, i - 1, j, k)] - ww1[f3(q.nx, q.ny, i + 1, j, k)]) *
            q.hx * dxxdx[i];
  }
}
__global__ void k_vel(const K q, const double* ru, const double* rv,
                      const double* rw, const double* ro, double* uu,
                      double* vv, double* ww) {
  long full = (long)q.nx * q.ny * q.nz;
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < full;
       t += (long)blockDim.x * gridDim.x) {
...
__global__ void k_adv(const K q, int im, const double* ru, const double* rv,
                      const double* rw, const double* uu, const double* vv,
                      const double* ww, double* fu, double* fv, double* fw,
                      const double* dxxdx, const double* dyydy,
                      const double* dzzdz) {
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    f[c] -= (m[f3(q.nx, q.ny, i, j + 1, k)] * vv[f3(q.nx, q.ny, i, j + 1, k)] -
             m[f3(q.nx, q.ny, i, j - 1, k)] * vv[f3(q.nx, q.ny, i, j - 1, k)]) * ty;
    f[c] -= (m[f3(q.nx, q.ny, i, j, k + 1)] * ww[f3(q.nx, q.ny, i, j, k + 1)] -
             m[f3(q.nx, q.ny, i, j, k - 1)] * ww[f3(q.nx, q.ny, i, j, k - 1)]) * tz;
  }
}
__global__ void k_ft_advect(const K q, const double* uu, const double* vv,
                            const double* ww, const double* tt, double* ft,
                            const double* dxxdx, const double* dyydy,
                            const double* dzzdz) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    double ty = q.hy * dyydy[j];
    double tz = q.hz * dzzdz[k];
    ft[c] = -1.0 * uu[c] * (tt[f3(q.nx, q.ny, i + 1, j, k)] -
                            tt[f3(q.nx, q.ny, i - 1, j, k)]) * q.hx * dxxdx[i];
    ft[c] -= vv[c] * (tt[f3(q.nx, q.ny, i, j + 1, k)] -
                      tt[f3(q.nx, q.ny, i, j - 1, k)]) * ty;
    ft[c] -= ww[c] * (tt[f3(q.nx, q.ny, i, j, k + 1)] -
                      tt[f3(q.nx, q.ny, i, j, k - 1)]) * tz;
  }
}
__global__ void k_ft_diffuse(const K q, const double* ro, const double* tt,
                             double* ft, const double* dxxdx,
                             const double* d2xxdx2, const double* dyydy,
                             const double* d2yydy2, const double* dzzdz,
                             const double* d2zzdz2, const double* rkapa,
                             const double* dkapa) {
...
template <int C>
__device__ void k_viscous(const K q, const double* uu, const double* vv,
                          const double* ww, double* fu, double* fv, double* fw,
                          const double* dxxdx, const double* d2xxdx2,
                          const double* dyydy, const double* d2yydy2,
...
__global__ void k_w1_w2_dx(const K q, const double* uu, const double* vv,
                           double* ww1, double* ww2, const double* dxxdx) {
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    long c = f3(q.nx, q.ny, i, j, k);
    ww1[c] = (uu[f3(q.nx, q.ny, i + 1, j, k)] - uu[f3(q.nx, q.ny, i - 1, j, k)]) *
             q.hx * dxxdx[i];
    ww2[c] = (vv[f3(q.nx, q.ny, i + 1, j, k)] - vv[f3(q.nx, q.ny, i - 1, j, k)]) *
             q.hx * dxxdx[i];
  }
}
__global__ void k_w3_dx(const K q, const double* ww, double* ww3,
                        const double* dxxdx) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    ww3[c] = (ww[f3(q.nx, q.ny, i + 1, j, k)] - ww[f3(q.nx, q.ny, i - 1, j, k)]) *
             q.hx * dxxdx[i];
  }
}
// FT += (C43*WW1^2+WW2^2+WW3^2)*RO*TMP ; FT -= OCV*WW1*TT
__global__ void k_ft_heat1(const K q, const double* ro, const double* tt,
                           double* ft, const double* ww1, const double* ww2,
                           const double* ww3) {
...
__global__ void k_cross_fufv(const K q, double* fu, double* fv,
...
__global__ void k_w1_w2_dy(const K q, const double* uu, const double* vv,
                           double* ww1, double* ww2, const double* dyydy) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    int i = q.i1 + (int)(t % (q.i2 - q.i1 + 1));
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    ww1[c] = (uu[f3(q.nx, q.ny, i, j + 1, k)] - uu[f3(q.nx, q.ny, i, j - 1, k)]) *
             q.hy * dyydy[j];
    ww2[c] = (vv[f3(q.nx, q.ny, i, j + 1, k)] - vv[f3(q.nx, q.ny, i, j - 1, k)]) *
             q.hy * dyydy[j];
  }
}
__global__ void k_ft_heat2(const K q, const double* ro, const double* tt,
                           double* ft, const double* ww1, const double* ww2) {
  const double tmp = q.ocv * q.ore;
...
__global__ void k_w1_dx_again(const K q, const double* uu, double* ww1,
                              const double* dxxdx) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
...
__global__ void k_w3_dz_full(const K q, const double* ww, double* ww3,
                             const double* dzzdz) {
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    int k = q.k1 + (int)(r / q.ny);
    long c = f3(q.nx, q.ny, i, j, k);
    ww3[c] = (ww[f3(q.nx, q.ny, i, j, k + 1)] - ww[f3(q.nx, q.ny, i, j, k - 1)]) *
             q.hz * dzzdz[k];
  }
}
__global__ void k_ft_heat3a(const K q, const double* ro, const double* tt,
                            double* ft, const double* ww1, const double* ww2,
                            const double* ww3) {
...
__global__ void k_w1_w2_dz(const K q, const double* uu, const double* vv,
                           double* ww1, double* ww2, const double* dzzdz) {
...
__global__ void k_w3_dy(const K q, const double* ww, double* ww3,
                        const double* dyydy) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    ww3[c] = (ww[f3(q.nx, q.ny, i, j + 1, k)] - ww[f3(q.nx, q.ny, i, j - 1, k)]) *
             q.hy * dyydy[j];
  }
}
__global__ void k_ft_heat3c(const K q, const double* ro, double* ft,
                            const double* ww1, const double* ww2,
                            const double* ww3) {
  const double tmp = q.ocv * q.ore;
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    ft[c] += (ww1[c] * ww1[c] + ww2[c] * ww2[c] + ww3[c] * ww3[c] +
              2.0 * ww2[c] * ww3[c]) * ro[c] * tmp;
  }
}
// FW += C13*ORE*((dWW1/dx)h + (dWW2/dy)h)  using current WW1 (=dU/dz), WW2(=dV/dz)
__global__ void k_fw_cross(const K q, double* fw, const double* ww1,
                           const double* ww2, const double* dxxdx,
                           const double* dyydy) {
  const double tmpc = q.c13 * q.ore;
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    long c = f3(q.nx, q.ny, i, j, k);
    fw[c] += tmpc *
             ((ww1[f3(q.nx, q.ny, i + 1, j, k)] - ww1[f3(q.nx, q.ny, i - 1, j, k)]) *
                  q.hx * dxxdx[i] +
              (ww2[f3(q.nx, q.ny, i, j + 1, k)] - ww2[f3(q.nx, q.ny, i, j - 1, k)]) *
                  q.hy * dyydy[j]);
  }
}
__global__ void k_w3_dx_2(const K q, const double* ww, double* ww3,
                          const double* dxxdx) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
...
__global__ void k_ft_heat3d(const K q, const double* ro, double* ft,
                            const double* ww1, const double* ww3) {
  const double tmp = q.ocv * q.ore;
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
...
// Rotation.
__global__ void k_rot(const K q, double* fu, double* fv, double* fw,
                      const double* ru, const double* rv, const double* rw) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    int i = q.i1 + (int)(t % (q.i2 - q.i1 + 1));
...
// ---------------------------------------------------------------------------
__global__ void k_step_uu(const K q, const double* ru, const double* rv,
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    double v2 = (ru[c] * ru[c] + rv[c] * rv[c] + rw[c] * rw[c]) *
                (1.0 / ro[c]) * (1.0 / ro[c]);
...
__global__ void k_step_dt(const K q, const double* ro, const double* uu,
                          const double* rkapa, const double* ddx,
                          const double* ddy, const double* ddz, double* ww1,
...
__global__ void k_save_state(const K q, const double* ru, const double* rv,
                             const double* rw, const double* ro, const double* tt,
                             double* zru, double* zrv, double* zrw, double* zro,
                             double* ztt) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    ...
    int k = q.k1 + (int)(r / (q.j2 - q.j1 + 1));
    long c = f3(q.nx, q.ny, i, j, k);
    zru[c] = ru[c]; zrv[c] = rv[c]; zrw[c] = rw[c];
    zro[c] = ro[c]; ztt[c] = tt[c];
  }
}
// update_prim: X = Z + coef*F  (5 primaries, interior)
__global__ void k_update_prim(const K q, double coef, const double* zru,
                              const double* zrv, const double* zrw,
                              const double* zro, const double* ztt,
                              const double* fu, const double* fv, const double* fw,
                              const double* fr, const double* ft, double* ru,
                              double* rv, double* rw, double* ro, double* tt) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    int i = q.i1 + (int)(t % (q.i2 - q.i1 + 1));
...
__global__ void k_mix_state(const K q, double coef, const double* ru,
                            const double* rv, const double* rw, const double* ro,
                            const double* tt, const double* fu, const double* fv,
                            const double* fw, const double* fr, const double* ft,
                            double* zru, double* zrv, double* zrw, double* zro,
                            double* ztt) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1) * (q.k2 - q.k1 + 1);
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
...
// ---------------------------------------------------------------------------
__global__ void k_bcon(const K q, const double* exx, const double* wyy,
                       const double* dzzdz, const double* rkapa, double* ru,
                       double* rv, double* rw, double* ro, double* tt,
                       double c13, double c23, double c43, double theta,
                       double tb, double tpr, double cln, double dz, int plume,
                       int itcon, int izcon, int top) {
  long n = (long)(q.i2 - q.i1 + 1) * (q.j2 - q.j1 + 1);
  int k = top ? q.k1 : q.k2;
  ...
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
    ...
    if (top) {
      ...
      if (!plume) {
        ...
        double e1 = exp(cln * ((exx[i] - q.xp) * (exx[i] - q.xp)));
        double e2 = exp(cln * ((wyy[j] - q.yp) * (wyy[j] - q.yp)));
        tt[c] = c43 * tt[cp] - c13 * tt[cp2] - c23 * dzloc *
                (theta - (theta - tpr) * e1 / q.hh * e2 / q.hh);
      }
    } else {
...
__global__ void k_pack_y(double* dst, const double* src, int nx, int ny, int j0,
                         int cnt, int nz) {
...
__global__ void k_unpack_y(double* dst, const double* src, int nx, int ny,
                           int j0, int cnt, int nz) {
  long n = (long)nx * cnt * nz;
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    long xn = t % nx;
...
__global__ void k_pack_x(double* dst, const double* src, int nx, int ny, int i0,
                         int cnt, int nz) {
...
__global__ void k_unpack_x(double* dst, const double* src, int nx, int ny,
                           int i0, int cnt, int nz) {
  long n = (long)cnt * ny * nz;
  for (long t = blockIdx.x * (long)blockDim.x + threadIdx.x; t < n;
       t += (long)blockDim.x * gridDim.x) {
    long xn = t % cnt;
...
__global__ void k_viscous_fu(const K q, const double* uu, const double* vv,
                             const double* ww, double* fu, double* fv,
                             double* fw, const double* dxxdx,
                             const double* d2xxdx2, const double* dyydy,
                             const double* d2yydy2, const double* dzzdz,
                             const double* d2zzdz2) {
  k_viscous<0>(q, uu, vv, ww, fu, fv, fw, dxxdx, d2xxdx2, dyydy, d2yydy2,
               dzzdz, d2zzdz2);
}
__global__ void k_viscous_fv(const K q, const double* uu, const double* vv,
                             const double* ww, double* fu, double* fv,
                             double* fw, const double* dxxdx,
                             const double* d2xxdx2, const double* dyydy,
                             const double* d2yydy2, const double* dzzdz,
                             const double* d2zzdz2) {
  k_viscous<1>(q, uu, vv, ww, fu, fv, fw, dxxdx, d2xxdx2, dyydy, d2yydy2,
               dzzdz, d2zzdz2);
}
__global__ void k_viscous_fw(const K q, const double* uu, const double* vv,
                             const double* ww, double* fu, double* fv,
                             double* fw, const double* dxxdx,
                             const double* d2xxdx2, const double* dyydy,
                             const double* d2yydy2, const double* dzzdz,
                             const double* d2zzdz2) {
  k_viscous<2>(q, uu, vv, ww, fu, fv, fw, dxxdx, d2xxdx2, dyydy, d2yydy2,
               dzzdz, d2zzdz2);
}
}  // namespace r3d
