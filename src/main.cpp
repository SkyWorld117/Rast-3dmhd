...
void write_gstate(const Params& p, const Derived& d, const Topology& t,
                  const SimState& s, const char* tag, int iter) {
  char fname[256];
  ...
  w(&magic, 4); w(&nf, 4); w(&n1, 4);
  int32_t hdr[13] = {d.nx, d.ny, d.nz,      t.mype, t.mypey, t.mypez,
                     s.nit, (int32_t)p.lmag, (int32_t)p.ixcon, (int32_t)p.iycon,
                     (int32_t)p.izcon,       (int32_t)p.itcon, (int32_t)p.ibcon};
  w(hdr, sizeof(hdr));
  double sc[27] = {s.dt,  s.timc, s.timt, s.timi,       s.umach, p.re,
                   d.repr, d.cv,  d.ocv,  d.ore,        p.theta, p.grav,
                   d.rkapst, d.orm, p.rm, d.obeta,       d.omx,   d.omz,
                   p.sf,   p.ampt, 0.0,    d.hx,         d.h2x,   d.hy,
                   d.h2y,  d.hz,   d.h2z};
  w(sc, sizeof(sc));
  ...
  const std::vector<double>* f1d[14] = {
      &s.exx, &s.dxxdx, &s.d2xxdx2, &s.ddx,  &s.wyy, &s.dyydy, &s.d2yydy2,
      &s.ddy, &s.zee,   &s.dzzdz,   &s.d2zzdz2, &s.ddz, &s.rkapa, &s.dkapa};
  for (int i = 0; i < 14; ++i) {
...
double initial_dt(const Params& p, const Derived& d, const Topology& t,
                  const SimState& s) {
  ...
  for (int k = k1; k <= k2; ++k)
    for (int j = 1; j <= ny - p.iy; ++j)
      for (int i = 1; i <= nx - p.ix; ++i) {
        ...
        wmin[1] = std::min(wmin[1],
                           0.5 * dd * dd * d.repr * d.cv * s.ro.at(i, j, k) /
                               s.rkapa[k]);
...
int main(int argc, char** argv) {
  ...
  if (p.pzp == 0.0) {
    ...
    for (int k = 0; k < d.nz; ++k) {
      double xe = s.zee[k];
      s.rkapa[k] =
          1.0 + (d.rkapst - 1.0) / 2.0 * (1.0 + std::tanh((xe - p.pzp) / p.sigma));
      s.dkapa[k] = (d.rkapst - 1.0) / 2.0 / p.sigma /
