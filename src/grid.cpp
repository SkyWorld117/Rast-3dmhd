...
namespace r3d {
    ...
    namespace {
        double smax_for(const Params& p, int icoord) {
            switch (icoord) {
                ...
                default: fprintf(stderr, "MKGRID: invalid ICOORD %d\n", icoord); exit(EXIT_FAILURE);
            }
        }
    }  // namespace
    ...
    void mkgrid(const Params& p, double scode, int icoord, double& sphys, double& dsds,
        ...
        if (p.ngrid == 2) {
            ...
            double sh = (1.0 - dpar) * onepi /
                        (dpar * atan((a - b) * c) + 2.0 * atan(b * c) - dpar * atan((a + b) * c));
            double sk =
                2.0 * c * onepi * smax /
                (2.0 * c * onepi +
                 sh * (2.0 * c *
                           (atan((a - b) * c) * (a - b) - atan((a + b) * c) * (a + b) +
                            atan((a - b - 1.0) * c) + atan((a + b - 1.0) * c) * (a + b - 1.0) +
                            atan((1.0 - a + b) * c) * (a - b)) -
                       log(1.0 + (a - b) * (a - b) * c * c) + log(1.0 + (a + b) * (a + b) * c * c) -
                       log(1.0 + (a + b - 1.0) * (a + b - 1.0) * c * c) +
                       log(1.0 + (1.0 - a + b) * (1.0 - a + b) * c * c)));
            dsds =
                1.0 /
                (sk * (1.0 + sh / onepi * (atan(c * (scode - a - b)) + atan(c * (a - scode - b)))));
    ...
    void xjacobi(const Params& p, int nx, double* exx, double* dxxdx, double* d2xxdx2,
                 double* ddx) {
    ...
    void build_grid_metrics(const Params& p, const Topology& t, SimState& s) {
        ...
        yjacobi(p, t.mypey, t.npey, s.d.ny, p.npy, s.d.nry, s.wyy.data(), s.dyydy.data(),
                s.d2yydy2.data(), s.ddy.data());
        zjacobi(p, t.mypez, t.npez, s.d.nz, p.npz, s.d.nrz, s.zee.data(), s.dzzdz.data(),
                s.d2zzdz2.data(), s.ddz.data());
    }
