...
#undef INT_FLAG
#undef DBL_FLAG
    if (flag("--lrot")) { p.lrot = parse_bool(take(i, "--lrot")); continue; }
    if (flag("--lmag")) { p.lmag = parse_bool(take(i, "--lmag")); continue; }
    if (flag("--lpot")) { p.lpot = parse_bool(take(i, "--lpot")); continue; }
    if (flag("--lrem")) { p.lrem = parse_bool(take(i, "--lrem")); continue; }
    if (flag("--lshr")) { p.lshr = parse_bool(take(i, "--lshr")); continue; }
    if (flag("--backend")) {
      std::string b = take(i, "--backend");
      if (b == "cpu") p.backend = Params::Backend::kCpu;
      else if (b == "gpu") p.backend = Params::Backend::kGpu;
      else usage_error("--backend must be 'cpu' or 'gpu'");
      continue;
    }
