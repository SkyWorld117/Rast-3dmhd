    ...
    void comm_mpi(MPI_Comm comm, const Params& p, const Topology& t, Field& f) {
        ...
        if (t.npez > 1) {
            ...
            if (t.mypez == 0) {
                ...
                MPI_Recv(f.data(), vcount, MPI_DOUBLE, t.mype - t.npey, tag1, comm,
                         MPI_STATUS_IGNORE);
                MPI_Send(f.data() + (long) ilaph * nx * ny, vcount, MPI_DOUBLE, t.mype - t.npey,
                         tag2, comm);
            } else {
                MPI_Sendrecv(f.data() + (long) (nz - p.ilap) * nx * ny, vcount, MPI_DOUBLE,
                             t.mype + t.npey, tag1, f.data(), vcount, MPI_DOUBLE, t.mype - t.npey,
                             tag1, comm, MPI_STATUS_IGNORE);
                MPI_Sendrecv(f.data() + (long) ilaph * nx * ny, vcount, MPI_DOUBLE, t.mype - t.npey,
                             tag2, f.data() + (long) (nz - ilaph) * nx * ny, vcount, MPI_DOUBLE,
                             t.mype + t.npey, tag2, comm, MPI_STATUS_IGNORE);
            }
        }
        ...
        if (t.npey > 1) {
            ...
            if (t.mypey == 0) {
                ...
                pack_y(f, ny - p.iy, iyh, sendbuf.data());
                MPI_Sendrecv(sendbuf.data(), ycount, MPI_DOUBLE, t.mype + 1, tag3, recvbuf.data(),
                             ycount, MPI_DOUBLE, t.mype - 1, tag3, comm, MPI_STATUS_IGNORE);
                unpack_y(f, 0, iyh, recvbuf.data());
                pack_y(f, iyh, iyh, sendbuf.data());
                MPI_Sendrecv(sendbuf.data(), ycount, MPI_DOUBLE, t.mype - 1, tag4, recvbuf.data(),
                             ycount, MPI_DOUBLE, t.mype + 1, tag4, comm, MPI_STATUS_IGNORE);
                unpack_y(f, ny - iyh, iyh, recvbuf.data());
            }
            ...
            if (t.mypey == 0) {
                pack_y(f, iyh, iyh, sendbuf.data());
                MPI_Send(sendbuf.data(), ycount, MPI_DOUBLE, t.mype + t.npey - 1, tag5, comm);
                MPI_Recv(recvbuf.data(), ycount, MPI_DOUBLE, t.mype + t.npey - 1, tag6, comm,
                         MPI_STATUS_IGNORE);
                unpack_y(f, 0, iyh, recvbuf.data());
            } else if (t.mypey == t.npey - 1) {
                MPI_Recv(recvbuf.data(), ycount, MPI_DOUBLE, t.mype - t.npey + 1, tag5, comm,
                         MPI_STATUS_IGNORE);
                unpack_y(f, ny - iyh, iyh, recvbuf.data());
                pack_y(f, ny - p.iy, iyh, sendbuf.data());
                MPI_Send(sendbuf.data(), ycount, MPI_DOUBLE, t.mype - t.npey + 1, tag6, comm);
            }
        } else {
